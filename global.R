...
{ # .. Temperature_DF   ----
  ...
  temp <- as.tbl( read_csv("Temp_weekly_summary.csv"))
  
  tempByIsland <- as.tbl(read_csv("Temp_weekly_summary_byIsl.csv")) 
  
  tempMonthly <- as.tbl(read_csv("Temp_monthly_summary.csv"))
  
  oni <- read_csv("nino34.csv") 
  
  oniWeekly <- read_csv("ENSO_Weekly.csv")
